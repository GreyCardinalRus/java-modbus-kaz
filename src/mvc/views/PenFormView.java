...
public class PenFormView extends JPanel// JFrame
{
	...
	void mainwork() {
		...
		if (isSimulation > 0) {
			...
			dataset.addValue(Math.sin(isSimulation * .1 + Math.random() * 0.1),
					"Tok 1 ", " " + (isSimulation - 1));
			dataset.addValue(
					Math.cos(isSimulation * .1 + Math.random() * 0.2 - 0.1),
					"Series cos", " " + (isSimulation - 1));
			dataset.addValue(Math.cos(isSimulation * .1 + Math.PI / 10),
					"Series Ideal", " " + (isSimulation - 1));
			if (isSimulation >= ((Integer) windowChartWidth.getValue())) {
				dataset.removeColumn(" "
						+ (isSimulation - ((Integer) windowChartWidth
								.getValue())));
				// System.out.println("Delete "+i);
			}
			listModel.addElement("Word " + isSimulation + "=" + isSimulation);
	...
	ActionListener taskPerformer = new ActionListener() {
		public void actionPerformed(ActionEvent evt) {
			...
			btnStartStop.setText(((isSimulation > 0) ? "Simulation... "
